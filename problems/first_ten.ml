...
let rec at k lst =
  ...
  | hd :: rst -> (
      ...
      | 1 -> hd
      | _ ->
          if k < 1 then raise (Invalid_k "k should be >= 1") else at (k - 1) rst
      )

(** [length] returns length of a list. naive implementation *)
let rec length = function [] -> 0 | _ :: rst -> 1 + length rst

(** [length'] returns length of a list. tail recursive *)
let length' lst =
  let rec aux acc = function [] -> acc | _ :: rst -> aux (acc + 1) rst in
  aux 0 lst
...
let rev' lst =
  let rec aux acc = function [] -> acc | hd :: rst -> aux (hd :: acc) rst in
  aux [] lst
...
let compress lst =
  let rec aux acc = function
    ...
    | [ x ] -> x :: acc
    | x :: y :: xs ->
        if x = y then aux acc (y :: xs) else aux (x :: acc) (y :: xs)
  in
  lst |> aux [] |> rev'
